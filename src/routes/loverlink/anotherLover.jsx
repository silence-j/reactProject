...
class Detail extends React.Component {
  ...
  getCode () {
    if (this.state.isPhone === false) {
      ...
      loginAPI.getCode({mobile: this.state.phone, type: 2}).then(res => {
        if(!res.data.code) {
          alert('发送成功')
        }
      })
  ...
  doBind () {
    ...
    this.props.history.push(`/payoff?phone=${this.state.phone}&captcha=${this.state.code}`)
  }
  // 取消绑定
  cancel() {
    ...
    return (
      <div className="L-container">
        ...
        <div className="login-type">
          ...
          <Input className={`${this.state.error ? 'errorBg' : 'phone'} `} placeholder="请输入情侣手机号码" value={this.state.phone} onChange={this.changePhone.bind(this, 'phone')}  onFocus={this.isRight.bind(this)} onBlur={this.orRight.bind(this, 'code')}/>
          ...
          <div className="operation">
            ...
            <Button className='cancel' onClick={this.cancel.bind(this)}>取消绑定</Button>
          </div>
