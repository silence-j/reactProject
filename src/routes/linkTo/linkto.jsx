    ...
    if (this.state.phone === '' && this.state.code === '') {
      ...
      if (this.state.isPhone !== true || this.state.code === '') {
        ...
        loginAPI.doBind({mobile: this.state.phone, code: this.state.code}).then((res) => {
          if(!res.data.code) {
            alert('绑定成功')
            if(qs.parse(this.props.location.search).origin === 'bind') {
              this.props.history.replace(`/homePage/center`)
            } else {
              this.props.history.replace(`/cardDetail?type=${localStorage.type}`)
            }
          } else {
